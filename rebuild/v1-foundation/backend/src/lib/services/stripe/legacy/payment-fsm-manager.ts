...
import { FSM_CONFIG } from '../config/constants';
import { 
...
// Types now imported from config/fsm-rules.ts
...
// ================================================
// FSM TRANSITION RULES (now imported from config/fsm-rules.ts)
// ================================================

// VALID_BOOKING_TRANSITIONS and VALID_PAYMENT_TRANSITIONS imported from config
...
export class PaymentFSMManager {
  ...
  private validateTransition(
    ...
    try {
      ...
      const context: StateValidationContext = {
        bookingId: 'temp-validation-context', // Not used for transition validation
        currentBookingStatus: fromBookingStatus,
        targetBookingStatus: toBookingStatus,
        currentPaymentStatus: fromPaymentStatus,
        targetPaymentStatus: toPaymentStatus,
        stripeEventType: 'validation-check' // Not used for basic transition validation
      };

      const result = this.stateValidator.validateTransition(context);
      if (!result.isValid) {
        throw new Error(result.errorMessage || `Invalid transition: ${fromBookingStatus}→${toBookingStatus}, ${fromPaymentStatus}→${toPaymentStatus}`);
      }
      ...
      if (!isValidBookingTransition(fromBookingStatus, toBookingStatus)) {
        throw new Error(
          `Invalid booking status transition: ${fromBookingStatus} → ${toBookingStatus}`
        );
      }

      if (!isValidPaymentTransition(fromPaymentStatus, toPaymentStatus)) {
        throw new Error(
  ...
  private async acquireTransitionLock(bookingId: string, stripeEventId: string): Promise<boolean> {
    try {
      ...
      const result = await this.redis.set(lockKey, lockValue, 'EX', FSM_CONFIG.LOCK_TIMEOUT_SECONDS, 'NX');
      return result === 'OK';
    } catch (error) {
