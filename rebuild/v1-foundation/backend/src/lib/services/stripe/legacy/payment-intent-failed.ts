...
import { createStripeErrorClassifier, type FailureClassification } from '../utils/error-classifier';
...
// ================================================
// FAILURE CLASSIFICATION (now imported from utils/error-classifier.ts)
// ================================================

// FailureClassification interface imported from utils/error-classifier.ts
...
export class PaymentIntentFailedHandler {
  ...
  private errorClassifier: ReturnType<typeof createStripeErrorClassifier>;

  constructor(postgres: Pool, redis: Redis) {
    this.postgres = postgres;
    this.redis = redis;
    this.fsmManager = createPaymentFSMManager(postgres, redis);
    this.errorClassifier = createStripeErrorClassifier();
  }
   ...
   * ✅ Now using new StripeErrorClassifier utils with fallback to legacy behavior
   */
  private classifyFailure(paymentIntent: any): FailureClassification {
    try {
      // Use new StripeErrorClassifier for enhanced classification
      return this.errorClassifier.classifyPaymentFailure(paymentIntent);
    } catch (newClassifierError) {
      // Fallback to legacy classification to ensure exact same behavior
      console.warn('🔄 Error classifier error, falling back to legacy classification:', newClassifierError);
