...
// ================================================
// CONFIGURATION (now imported from config/constants.ts)
// ================================================

// IDEMPOTENCY_CONFIG imported from ../config/constants
...
export class StripeIdempotencyManager {
  ...
  async getIdempotencyStats(): Promise<{
    ...
    try {
      ...
      pipeline.eval(`
        local idempotency_pattern = "${IDEMPOTENCY_CONFIG.KEY_PREFIX}:*"
        local processing_pattern = "${IDEMPOTENCY_CONFIG.LOCK_PREFIX}:*"
  ...
  private getIdempotencyKey(idempotencyKey: string): string {
    return `${IDEMPOTENCY_CONFIG.KEY_PREFIX}:${idempotencyKey}`;
  }

  private getProcessingLockKey(stripeEventId: string): string {
    return `${IDEMPOTENCY_CONFIG.LOCK_PREFIX}:${stripeEventId}`;
  }

  private getResponseCacheKey(idempotencyKey: string): string {
    return `${IDEMPOTENCY_CONFIG.CACHE_PREFIX}:${idempotencyKey}`;
  }
}
